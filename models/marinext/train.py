...
from marinext_wrapper import GenericModel

sys.path.append(os.path.join(up(up(os.path.abspath(__file__))), 'utils'))
from dataset import MADOS, gen_weights, class_distr, MARIDADataset
from vscp import VSCP
from metrics import Evaluation
from metrics import Evaluation, confusion_matrix
from assets import bool_flag, cosine_scheduler, labels
...
def main(options):
    ...
    if os.path.exists(os.path.join(options['path'], 'patches')):
        dataset_train = MARIDADataset(options['path'], splits_path, 'train')
        dataset_val = MARIDADataset(options['path'], splits_path, 'val')
        options['dataset_name'] = "MARIDA"
    else:
        dataset_train = MADOS(options['path'], splits_path, 'train')
        dataset_val = MADOS(options['path'], splits_path, 'val')
        options['dataset_name'] = "MADOS"
    ...
    model = GenericModel(options['input_channels'], options['output_channels'], options['config'])
    ...
    if options['mode']=='train':
        ...
        for epoch in range(start, epochs+1):
            ...
            if (epoch % eval_every == 0 or epoch==1) and options['model_ema'] and options['model_ema_eval']:
                ...
                # Dump final metrics and confusion matrix to text file
                conf_mat = confusion_matrix(y_true_val_ema, y_predicted_val_ema, labels, percentage=False)
                model_name = os.path.basename(options['config']).replace('.py', '')
                dataset_name = options.get('dataset_name', 'MADOS')
                out_txt = os.path.join(root_path, f"dl_results_{model_name}_{dataset_name}.txt")
                with open(out_txt, 'w') as f:
                    f.write(f"Results after epoch {epoch}:\n")
                    f.write(str(acc_val_ema) + "\n\n")
                    f.write("Confusion Matrix:\n")
                    f.write(str(conf_mat.to_string()) + "\n")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--config', required=True, type=str, help='Path to model config file')
