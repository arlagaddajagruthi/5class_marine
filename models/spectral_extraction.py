...
def ImageToDataframe(RefImage, cols_mapping = {}, keep_annotated = True, prefix = '_rhorc_'):
    ...
    # Read patch
    with rasterio.open(RefImage) as ds:
        IM = np.copy(ds.read())

    # Read associated class patch
    with rasterio.open(RefImage.replace(prefix , '_cl_')) as ds_cl:
        IM_cl = np.copy(ds_cl.read(1))[np.newaxis, :, :]

    # Read associated confidence level patch
    with rasterio.open(RefImage.replace(prefix , '_conf_')) as ds_conf:
        IM_conf = np.copy(ds_conf.read(1))[np.newaxis, :, :]
    
    # Read associated class patch
    with rasterio.open(RefImage.replace(prefix , '_rep_')) as ds_rep:
        IM_rep = np.copy(ds_rep.read(1))[np.newaxis, :, :]
