                ...
                for crop in splits:
                    ...
                    if crop_name in self.ROIs_split:
                        ...
                        with rasterio.open(cl_path) as ds:
                            temp = np.copy(ds.read(1).astype(np.int64))
...
class MARIDADataset(Dataset):
    def __init__(self, path, splits, mode='train'):
        ...
        patches = glob(os.path.join(path, 'patches', '**', '*.tif'), recursive=True)
        ...
        for p in tqdm(patches, desc='Load MARIDA '+mode+' set to memory'):
            basename = os.path.basename(p).split('.tif')[0]
            img_name = basename.replace('S2_', '')
            if img_name in self.ROIs_split:
                with rasterio.open(p) as src:
                    img = src.read()
                self.X.append(img)
                
                cl_path = p.replace('.tif', '_cl.tif')
                with rasterio.open(cl_path) as src_cl:
                    mask = src_cl.read(1).astype(np.int64)
                self.y.append(mask)
