...
class MarineLGBMClassifier:
    """
    LightGBM Gradient Boosting Classifier Pipeline for 5-Class Marine Debris & Aquatic Target Classification.
    Uses highly optimized histogram-based decision tree algorithms with multi-threading.
    """
    def __init__(
        self,
        n_estimators=100,
        ...
        if not HAS_LIGHTGBM:
            raise ImportError(
                "LightGBM is not installed. Please install it via 'pip install lightgbm' or 'conda install -c conda-forge lightgbm'."
            )
    ...
    def save(self, filepath):
        ...
        if filepath.endswith('.txt') or filepath.endswith('.model'):
...
def run_lightgbm_pipeline(dataset_name, dataset_dir, max_train_pixels=500000, n_estimators=100, max_depth=6, num_leaves=31, learning_rate=0.1):
    """
    End-to-end training, evaluation, and artifact saving pipeline for LightGBM.
    """
    ...
    print(f"\n=======================================================")
    print(f" Running LightGBM Pipeline on {dataset_name} Dataset")
    print(f"=======================================================\n")
    ...
    print(f"Training LightGBM on {len(y_train)} pixels with {X_train.shape[1]} features...")
    ...
    train_time = lgb_clf.fit(X_train, y_train)
    print(f"Training completed in {train_time:.2f} seconds.")
    
    # Evaluation
    print("Evaluating test set...")
    y_pred, inf_time = lgb_clf.predict(X_test)
    ...
    model_path = os.path.join(out_dir, f"{dataset_name.lower()}_lgbm_model.joblib")
    lgb_clf.save(model_path)
    ...
    results_str += f"Training LightGBM on {len(y_train)} pixels with {X_train.shape[1]} features...\n"
    ...
    txt_filename = f"{dataset_name.lower()[:5]}_lgbm_results.txt"
    txt_filepath = os.path.join(repo_root, txt_filename)
    with open(txt_filepath, "w", encoding="utf-8") as f:
        f.write(results_str)
        
    table_base_path = os.path.join(log_dir, f"{dataset_name}_lgbm_metrics")
    save_metrics_table(metrics, CLASS_NAMES, table_base_path)
    
    cm_plot_path = os.path.join(log_dir, f"{dataset_name}_lgbm_confusion_matrix.png")
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Run LightGBM Pipeline on Marine Datasets")
    ...
    if args.dataset in ["MADOS_5Class", "both"] and os.path.exists(mados_path):
        run_lightgbm_pipeline("MADOS_5Class", mados_path, max_train_pixels=args.max_pixels, n_estimators=args.n_estimators, max_depth=args.max_depth, num_leaves=args.num_leaves, learning_rate=args.learning_rate)

    if args.dataset in ["MARIDA_5Class", "both"] and os.path.exists(marida_path):
        run_lightgbm_pipeline("MARIDA_5Class", marida_path, max_train_pixels=args.max_pixels, n_estimators=args.n_estimators, max_depth=args.max_depth, num_leaves=args.num_leaves, learning_rate=args.learning_rate)
