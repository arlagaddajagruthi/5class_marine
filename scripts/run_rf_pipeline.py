...
from src.models.xgb_classifier import extract_pixel_features_21
...
class MarineRFClassifier21:
    def __init__(self, n_estimators=100, max_depth=None, num_classes=5):
...
def evaluate_rf_on_dataset(dataset_name, dataset_dir, max_train_pixels=500000):
    print(f"\n=======================================================")
    print(f" Running Random Forest (21 features) on {dataset_name} Dataset")
    print(f"=======================================================\n")
    ...
    print(f"Training RF on {len(y_train)} pixels with {X_train.shape[1]} features...")
    ...
    # Initialize RF model
    num_trees = 100
    rf_clf = MarineRFClassifier21(n_estimators=num_trees, num_classes=5)
    
    # Train model
    train_time = rf_clf.fit(X_train, y_train)
    print(f"Training completed in {train_time:.2f} seconds.")
    
    # Evaluate model
    print("Evaluating test set...")
    y_pred, inf_time = rf_clf.predict(X_test)
    ...
    model_json_path = os.path.join(repo_root, "outputs", f"{dataset_name.lower()}_rf21_model.joblib")
    rf_clf.save(model_json_path)
    ...
    results_str += f"Training RF on {len(y_train)} pixels with {X_train.shape[1]} features...\n"
    ...
    txt_filename = f"{dataset_name.lower()[:5]}_rf21_results.txt"
    txt_filepath = os.path.join(repo_root, txt_filename)
    with open(txt_filepath, "w", encoding="utf-8") as f:
        f.write(results_str)
        
    # Save CSV and MD tables
    table_base_path = os.path.join(out_log_dir, f"{dataset_name}_rf21_metrics")
    save_metrics_table(metrics, CLASS_NAMES, table_base_path)
    
    # Save Confusion Matrix Plot
    cm_plot_path = os.path.join(out_log_dir, f"{dataset_name}_rf21_confusion_matrix.png")
...
if __name__ == "__main__":
    ...
    mados_summary, mados_txt = evaluate_rf_on_dataset("MADOS_5Class", mados_path)
    marida_summary, marida_txt = evaluate_rf_on_dataset("MARIDA_5Class", marida_path)
    
    print("\n=======================================================")
    print(" SUMMARY OF RF EXPERIMENTS")
    print("=======================================================")
